...
@deprecated
class OT_GZipMiddleware:
    # noinspection PyDefaultArgument
    def __init__(
        self,
        app: ASGIApp,
        minimum_size: int = 500,
        exlude_routes: List[Union[str, Pattern]] = [],
    ) -> None:
        self.app = app
        self.minimum_size = minimum_size
        self.exlude_routes = exlude_routes
    ...
    async def __call__(self, scope: Scope, receive: Receive, send: Send) -> None:
        if scope["type"] == "http":
            ...
            if "gzip" in headers.get("Accept-Encoding", ""):
                gzip = True
                for exlude in self.exlude_routes:
                    if isinstance(exlude, str):
                        if scope["path"] == exlude:
                            gzip = False
                            break
                if gzip:
                    responder = OT_GZipResponder(self.app, self.minimum_size)
                    await responder(scope, receive, send)
                    return
        await self.app(scope, receive, send)


class OT_GZipResponder:
